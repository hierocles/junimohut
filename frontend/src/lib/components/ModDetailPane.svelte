<script lang="ts">
  ...
  import { dependencyRowsForMod, dependentRowsForMod } from '$lib/mods/dependencies';
  import {
  ...
  const dependencyRows = $derived(
    mod ? dependencyRowsForMod(mod, mods) : [],
  );
  const hasDependencies = $derived(dependencyRows.length > 0);
  const dependentRows = $derived(
    mod ? dependentRowsForMod(mod, mods) : [],
  );
  const dependencyIssueCount = $derived(mod?.missingDependencyCount ?? mod?.dependencyIssues?.length ?? 0);
  ...
  const canDownload = $derived(
    mod != null &&
      (mod.updateStatus?.state === 'update' || mod.updateStatus?.state === 'update_available') &&
      (mod.manifest?.UpdateKeys?.some((k) => k.startsWith('Nexus:')) ?? false),
  );

  const modStatus = $derived(mod ? modStatusInfo(mod) : { text: '', badge: '' });
  ...
  function modName(m: Mod): string {
    return m.manifest?.Name ?? m.folderPath;
  }
  ...
  function dependencyStatusBadge(state: 'satisfied' | 'missing' | 'version_too_low' | 'disabled' | 'optional'): {
    text: string;
    badge: string;
  } {
    if (state === 'satisfied') {
      return { text: dependencyInstalled(), badge: 'state-badge state-badge--success' };
    }
    if (state === 'optional') {
      return { text: dependencyOptionalAbsent(), badge: 'state-badge state-badge--muted' };
    }
    if (state === 'version_too_low') {
      return { text: dependencyVersionTooLow(), badge: 'state-badge state-badge--error' };
    }
    if (state === 'disabled') {
      return { text: dependencyDisabled(), badge: 'state-badge state-badge--error' };
    }
    return { text: dependencyNotInstalled(), badge: 'state-badge state-badge--error' };
  }
  ...
  const tabs = $derived(
    [
      ...
      ...(hasDependencies ? [{ id: 'dependencies' as const, label: 'Dependencies' }] : []),
      ...(hasDependents ? [{ id: 'dependents' as const, label: 'Dependents' }] : []),
...
{#if mod}
  ...
  >
    ...
    <div class="detail-body type-ui" role="tabpanel" style="padding: var(--space-4);">
      {#key activeTab}
        <div class="detail-tab-panel motion-fade-in">
      {#if activeTab === 'general'}
        <div class="summary-row grid sm:grid-cols-2 lg:grid-cols-4" style="gap: var(--space-4); margin-bottom: var(--space-4);">
          <div class="min-w-0">
            <span class="field-label type-label">Name</span>
            <p class="summary-name type-subhead truncate text-surface-50">{modName(mod)}</p>
          </div>
          <div class="min-w-0">
          ...
          <div class="min-w-0 sm:col-span-2 lg:col-span-2">
            ...
            <div class="flex min-w-0 items-center gap-1">
              <p class="type-mono truncate text-surface-300">{mod.manifest?.UniqueID || '—'}</p>
          ...
          <div class="min-w-0 lg:col-span-2">
            ...
            <div class="flex min-w-0 items-center gap-1">
              <p class="type-mono truncate text-surface-400">{mod.absolutePath}</p>
              <button
                type="button"
                class="copy-btn"
        ...
        <div class="info-columns">
            ...
            <dl class="field-list">
                ...
                <div class="field-row">
                  ...
                  <dd class="type-mono">
                    ...
                    {#if mod.manifest.ContentPackFor.MinimumVersion}
                      <span class="text-surface-500"> ≥ {mod.manifest.ContentPackFor.MinimumVersion}</span>
                    {/if}
                  </dd>
                </div>
              {/if}
              {#if mod.manifest?.Description?.trim()}
                <div class="field-row field-row-block">
                  <dt>Description</dt>
                  <dd class="type-prose type-meta text-surface-300">{mod.manifest.Description}</dd>
                </div>
              {/if}
            </dl>
          </section>
                ...
                <div class="field-row field-row-block">
                  ...
                  <dd class="flex flex-wrap gap-1">
                    {#each modCategories as cat (cat.id)}
                      <span class="category-tag chip-colored" style:--chip-color={cat.color || 'var(--color-primary-500)'}>
                        <span class="dot" style:background={cat.color || 'var(--color-primary-500)'}></span>
                        {cat.name}
                      </span>
                    {/each}
                  </dd>
          ...
          <section class="info-col">
            ...
            <dl class="field-list">
              ...
              <div class="field-row">
                <dt>Key</dt>
                <dd class="type-mono text-surface-400">{mod.groupKey || '—'}</dd>
              </div>
            </dl>
          </section>
          ...
          {#each dependencyRows as row (row.uniqueID + (row.isContentPack ? ':cp' : ''))}
            ...
            <li class="dep-row">
              <div class="dep-row-main min-w-0">
                <span class="type-mono text-surface-200">{row.uniqueID}</span>
                {#if row.minimumVersion}
                  <span class="text-surface-500">≥ {row.minimumVersion}</span>
                {/if}
                ...
                {:else}
                  <span class="type-caption type-meta">{dependencyLoadOrderLabel()}</span>
                {/if}
              </div>
              <div class="dep-row-status flex flex-wrap items-center gap-2">
                ...
                {#if row.installedName && row.state !== 'missing' && row.state !== 'optional'}
                  <span class="type-meta text-surface-400">
                    {row.installedName}{#if row.installedVersion} v{row.installedVersion}{/if}
                  </span>
                {/if}
                <div class="dep-row-actions flex flex-wrap items-center gap-1">
          ...
          {#each dependentRows as row (row.modId)}
            <li class="dep-row">
              <div class="dep-row-main min-w-0">
                ...
                {#if row.uniqueID}
                  <span class="type-mono text-surface-400">{row.uniqueID}</span>
                {/if}
                {#if row.version}
                  <span class="text-surface-500">v{row.version}</span>
                {/if}
                {#if row.minimumVersion}
                  <span class="text-surface-500">requires ≥ {row.minimumVersion}</span>
                {/if}
                {#if row.isContentPack}
                  <span class="type-caption type-meta">content pack</span>
                {:else if row.isRequired}
              ...
              <div class="dep-row-status flex flex-wrap items-center gap-2">
                {#if row.enabled}
                  <span class="state-badge state-badge--success">Enabled</span>
                {:else}
      ...
      {:else if activeTab === 'update'}
        <dl class="field-list max-w-2xl">
          ...
          {#if mod.updateStatus?.message?.trim()}
            <div class="field-row field-row-block">
              <dt>Note</dt>
              <dd class="type-prose type-meta text-surface-300">{mod.updateStatus.message}</dd>
            </div>
          {/if}
...
<style>
  ...
  .detail-tab:focus-visible {
    outline: 2px solid color-mix(in oklab, var(--color-primary-500) 50%, transparent);
    outline-offset: -2px;
  }
  ...
  .copy-btn:hover {
    color: var(--color-surface-50);
    background-color: color-mix(in oklab, var(--color-surface-800) 100%, transparent);
  }
  ...
  .dep-row {
    ...
    gap: var(--space-2);
    padding: var(--space-2) var(--space-3);
    background-color: color-mix(in oklab, var(--color-surface-800) 50%, transparent);
    border-radius: var(--radius-base, 0.25rem);
  }
