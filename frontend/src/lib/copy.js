...
function sanitizeNetworkError(message) {
    if (/could not resolve Nexus server|could not connect to Nexus/i.test(message)) {
        return message;
    }
...
export function installCompleteLine(ok, fail) {
    ...
    if (ok === 1)
        return "One more mod for the collection.";
...
export const aboutDialogDisclaimer = "Not affiliated with ConcernedApe or Chucklefish.";
...
export const reinstallSavedDeleteOldLabel = "Remove old mod files first (keeps config.json and manifest.json)";
export const reinstallSavedMissingError = "No saved download found for this mod.";
...
export const setupWelcomeBody = "Tell Junimo Hut where Stardew Valley lives and where to store mod files. Enabled mods are symlinked into your game's Mods folder.";
export const settingsNexusHint = "Paste an API key from your Nexus Mods account to download updates in Junimo Hut.";
...
export const downloadNoArchivePath = "Download finished but no archive path was returned.";
export const downloadCompleteReview = "Download complete. Review the install dialog.";
...
export const downloadsEmptyHistory = "No saved archives yet. Finished Nexus downloads appear here for install or reinstall.";
export const downloadsActiveEmpty = "Nothing downloading. Nexus downloads show progress here.";
...
export const downloadsLoadError = "Couldn't load saved archives. Check Settings paths or your connection, then try again.";
...
export const downloadsUnlinkedHint = "Install once so future archives match by manifest UniqueID or Nexus mod ID.";
...
export const downloadsBulkLearnHint = "Ctrl+click to select · Shift+click for a range · Enter on a focused row installs · Esc to clear";
export const downloadsBulkClearSelection = "Clear selection";
export const downloadsBulkRangeHint = "Shift+click another row to add to the selection";
...
export const downloadsBulkReinstallConfirmTitle = "Reinstall selected archives?";
...
export const nexusKeyRejected = "Nexus API key was rejected. Copy a valid key from your Nexus account settings.";
export const registerNxmSuccess = "Registered nxm:// links to open in Junimo Hut.";
export const smapiInstallerOpened = "Opened the SMAPI installer. Run it to install or update SMAPI.";
...
export const deleteModDeleteArchiveNoneHint = "No selected mods have a saved archive";
...
export function missingDependencyBadge(count, issues) {
    ...
    if (count === 1) {
        if (hasVersionIssue)
            return "Dependency version too low";
        if (hasDisabled)
            return "Dependency disabled";
        return "Missing dependency";
    }
    if (hasVersionIssue || hasDisabled)
        return `${count} dependency issues`;
...
export const settingsPathsHint = "Enabled mods are symlinked from your mod library into the game Mods folder.";
...
export const gridTagsLearnHint = "to assign labels · Filter the list with tag toggles in the sidebar";
...
export const tagsSidebarEmptyHint = "Tags label mods and filter the list. Click a Tags cell in the grid to assign labels.";
export const tagsSidebarCreateFirst = "Create your first tag";
export const tagsSidebarFooterHint = "Toggle filters to narrow the mod list. Click a Tags cell to assign labels.";
...
export function normalizeArchivePaths(paths) {
    ...
    for (const raw of paths) {
        const path = raw.trim();
        if (!path || !isArchivePath(path) || seen.has(path))
            continue;
        seen.add(path);
        out.push(path);
    }
