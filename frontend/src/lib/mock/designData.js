import { resolveDependencies, countModsWithDependencyIssues, } from "$lib/mods/dependencies";
import { pathBasename } from "$lib/copy";
import { DEFAULT_TAG_IDS, buildDefaultCategories, } from "$lib/mods/defaultTags";
...
const MOD_SEEDS = [
    // Quality of Life (6)
    {
        ...
        author: "Candidus42",
        version: "1.47.0",
        description: "Shows live info about anything you hover over or hold a button on.",
        category: "qol",
        hasConfig: true,
        nexusId: "509",
        savedDownloadPath: "C:/Users/Example/AppData/JunimoHut/downloads/LookupAnything.zip",
    },
    {
    ...
    },
    {
        ...
        author: "Pathoschild",
        version: "4.20.0",
        description: "Buy a tractor to more efficiently work with crops, clear twigs and rocks, and more.",
        category: "farming",
        hasConfig: true,
        update: {
...
function groupForMod(folderPath, grouping = "folder") {
...
function buildMod(seed, enabledOverrides) {
...
function buildCategories(mods) {
    const byCategory = Object.fromEntries(Object.values(DEFAULT_TAG_IDS).map((id) => [id, []]));
...
function allMods() {
    return resolveDependencies(MOD_SEEDS.map((seed) => buildMod(seed, enabledOverrides)));
}
export function filterMods(mods, search, hideDisabled) {
    ...
    return mods.filter((m) => {
        ...
        if (!q)
            return true;
        const hay = `${m.manifest?.Name} ${m.manifest?.Author} ${m.manifest?.UniqueID} ${m.folderPath}`.toLowerCase();
...
export function getMockRefreshData(search, hideDisabled) {
    const mods = allMods();
    const categories = buildCategories(mods);
    const filtered = filterMods(mods, search, hideDisabled);
    const readyCount = filtered.filter((m) => m.updateStatus?.state === "update_available" ||
        m.updateStatus?.state === "update").length;
...
export function setMockModEnabled(modId, enabled) {
    enabledOverrides.set(modId, enabled);
}
export function getMockSavedDownloads() {
    const now = Math.floor(Date.now() / 1000);
    return [
        {
            archivePath: "C:/Users/Example/AppData/JunimoHut/downloads/LookupAnything.zip",
        ...
        {
            archivePath: "C:/Users/Example/AppData/JunimoHut/downloads/ContentPatcher.zip",
        ...
        {
            archivePath: "C:/Users/Example/AppData/JunimoHut/downloads/orphan-pack.zip",
...
//# sourceMappingURL=designData.js.map