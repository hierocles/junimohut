...
const MOCK_NEXUS_MOD_CATEGORIES: Record<number, string> = {
  509: "User Interface",
...
export function nexusModIdFromUpdateKey(key: string): number | null {
  if (!key.startsWith("Nexus:")) return null;
  const id = Number.parseInt(key.slice("Nexus:".length), 10);
  return Number.isFinite(id) && id > 0 ? id : null;
}

export function tagIdForNexusCategory(name: string): string {
