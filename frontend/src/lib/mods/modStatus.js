...
export function modStatusInfo(mod) {
    ...
    const issueCount = mod.missingDependencyCount ?? mod.dependencyIssues?.length ?? 0;
    if (issueCount > 0) {
...
export function modStatusSortKey(mod) {
    const state = mod.updateStatus?.state;
    if (state === "update" || state === "update_available")
        return 0;
    if (state === "incompatible")
...
//# sourceMappingURL=modStatus.js.map